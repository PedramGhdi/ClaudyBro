...
- **Idle helper cleanup** — One-shot helpers a CLI leaves running (`npm`, `node`, shell pipelines) are killed after 90 seconds of inactivity (configurable, 0 to disable). MCP servers are exempt: idling is what they do between tool calls, and no CLI silently respawns a killed one — Claude Code drops that server's tools from the running session and tells the model they're gone, until you run `/mcp reconnect`.

- **MCP-aware monitoring** — ClaudyBro identifies MCP servers by inspecting command-line args via `KERN_PROCARGS2` — Shadcn, Brave Search, Playwright, Context7, and any `@scope/mcp-server-*` package are recognised. These are tagged with a green **MCP** badge in the process inspector, never idle-killed while their CLI is running, and excluded from orphan detection. When the CLI exits, its whole subtree — MCP servers included — is cleaned up after a 15-second grace period, so bouncing the CLI doesn't lose them.
...
| **Idle helper cleanup** | Idle CLI helpers killed after 90s (configurable, 0 to disable); live MCP servers exempt until the CLI exits |
...
| **Settings** | Font size and family, CLI binary paths, orphan/auto-kill timeouts, idle helper timeout |
...
7. Command-line args are inspected via `KERN_PROCARGS2` — MCP servers and language servers are hidden from the orphan panel, because being idle is their normal state. A server attached to the running CLI is left alone entirely; one left behind by an earlier run is not attached, and is still reaped
8. When the CLI exits, everything ever seen in its subtree is killed after a 15-second grace period — matched on PID *and* start time, so a recycled PID can't inherit a death sentence
